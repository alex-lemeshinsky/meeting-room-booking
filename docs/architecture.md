...
3. перевіряє активне наступне бронювання тієї самої кімнати, де
   `next.start_at = current.end_at`, і блокує обидва рядки через `FOR SHARE`;
4. вставляє notification з унікальним idempotency constraint;
5. у тій самій транзакції ставить ідентифікатор notification у чергу через
   PostgreSQL `pg_notify`; PostgreSQL віддає подію слухачам лише після commit;
6. кожен API instance слухає канал через PostgreSQL `LISTEN` і локально
   передає отриману подію своїм SSE-клієнтам.

У межах одного циклу використовується transaction-level
`pg_try_advisory_xact_lock`; транзакція охоплює пошук, блокування рядків,
вставлення сповіщень і публікацію `pg_notify`.

Блокування `FOR SHARE` на поточному й наступному бронюваннях серіалізує
скасування відносно рішення про доставку (`NOTIFY-03`). Якщо скасування
комітиться першим, scheduler бачить неактивний рядок і не створює сповіщення.
Якщо першим комітиться scheduler, рішення про доставку вже прийняте на
активних бронюваннях, а скасування чекає короткої транзакції scheduler.

SSE дає швидку доставку відкритій вкладці незалежно від API instance, до
якого вона підключена. PostgreSQL залишається джерелом істини: якщо користувач
був offline, SSE розірвався або relay тимчасово недоступний, непрочитане
