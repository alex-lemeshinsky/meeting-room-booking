...
function TestComponent({
  ...
  return (
    <button onClick={() => showToast({ message, type })}>Показати toast</button>
  );
}
...
describe("ToastProvider", () => {
  ...
  it("renders toast message in status region when triggered", () => {
    ...
    expect(
      within(statusRegion).getByText("Тестове сповіщення")
    ).toBeInTheDocument();
  });
  ...
  it("automatically dismisses toast after 8 seconds (8000ms)", () => {
    ...
    expect(
      within(statusRegion).queryByText("Тестове сповіщення")
  ...
  it("limits visible toasts to max 3, removing the oldest one when 4th is added", () => {
    ...
    expect(
      within(statusRegion as HTMLElement).getByText("Toast 1")
    ).toBeInTheDocument();
    expect(
      within(statusRegion as HTMLElement).getByText("Toast 2")
    ).toBeInTheDocument();
    expect(
      within(statusRegion as HTMLElement).getByText("Toast 3")
    ).toBeInTheDocument();

    fireEvent.click(screen.getByRole("button", { name: "Btn 4" }));

    expect(
      within(statusRegion as HTMLElement).queryByText("Toast 1")
    ).not.toBeInTheDocument();
    expect(
      within(statusRegion as HTMLElement).getByText("Toast 2")
    ).toBeInTheDocument();
    ...
    expect(
      within(statusRegion as HTMLElement).getByText("Toast 4")
    ).toBeInTheDocument();
  });
