...
Поточний слайс реалізує Stage 9: сповіщення про завершення бронювань (Notifications). Система автоматично виявляє ситуації, коли поточне бронювання користувача завершується і в цій же кімнаті одразу (без часового проміжку) починається наступне бронювання. Фоновій інспекції передує отримання PostgreSQL transactional advisory lock (`pg_try_advisory_xact_lock`), що гарантує ідемпотентне та єдине виконання фонового шедулера в багатозадачному середовищі API.

При виявленні кандидату створюється сповіщення для власника поточного бронювання з українським текстом повідомлення, а також надсилається подія в SSE push-потік (`/api/v1/notifications/stream`). Шапка застосунку (`ProtectedHeader`) містить іконку дзвіночка (`NotificationBell`) з бейджем кількості непрочитаних сповіщень. При натисканні на дзвіночок відкривається панель сповіщень (`NotificationPanel`), а клік по сповіщенню позначає його як прочитане (`PATCH /api/v1/notifications/:id/read`), очищує бейдж і перенаправляє на сторінку `Мої бронювання`.

Також збережено всі можливості попередніх етапів: реєстрацію з підтвердженням email, вхід, сесії, вихід, захист маршрутів, список кімнат з фільтром за місткістю, тижневий розклад, повторювані щотижневі бронювання та сторінку `Мої бронювання` з історією та cursor-пагінацією.

Персоналізація першого дня тижня в календарі (`SETTINGS-01`) залишається єдиним наступним етапом.

Реалізовані офіційні бонуси на цьому етапі: Docker Compose з автоматичними міграціями та seed, фільтр кімнат за місткістю, інтеграційні API-тести, захист від конкурентного подвійного бронювання, повноцінний mobile-сценарій, підтвердження email у dev-режимі, повторювані бронювання та ідемпотентні сповіщення реального часу із транзакційними блокуваннями й SSE. Персоналізація першого дня тижня ще не реалізована.
