...
Поточний слайс реалізує Stage 8: повторювані щотижневі бронювання (Weekly Recurring Bookings). Користувач може створювати як одноразові бронювання, так і серії від 2 до 52 повторень із тижневим інтервалом. Серія розраховується за локальним стінним часом `Europe/Kyiv` із збереженням локального часу початку й завершення навіть під час сезонного переходу DST. Створення серії є атомарною операцією `all-or-nothing`: якщо хоча б одне повторення перетинається з існуючим бронюванням, вся серія відхиляється з помилкою `409 BOOKING_CONFLICT` із вказанням номера першого конфліктного повторення.

Календар розкладу та сторінка `Мої бронювання` позначають серійні події іконкою повторення `↻` та індикатором `Частина повторюваної серії (N з M)`. При скасуванні події з серії користувач може обрати скасування лише однієї конкретної події (`Лише цю подію`) або всієї серії (`Усю серію`). При скасуванні всієї серії скасовуються лише ті її події, які ще не завершилися на момент запиту.

Також збережено всі можливості попередніх етапів: реєстрацію з підтвердженням email, вхід, сесії, вихід, захист маршрутів, список кімнат з фільтром за місткістю, тижневий розклад, сторінку `Мої бронювання` з історією та cursor-пагінацією.

Сповіщення та персоналізація першого дня тижня залишаються наступними етапами.

Реалізовані офіційні бонуси на цьому етапі: Docker Compose з автоматичними міграціями та seed, фільтр кімнат за місткістю, інтеграційні API-тести, захист від конкурентного подвійного бронювання, повноцінний mobile-сценарій, підтвердження email у dev-режимі та повторювані бронювання. Сповіщення та персоналізація першого дня тижня ще не реалізовані.
