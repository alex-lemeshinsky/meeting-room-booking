...
Поточний слайс реалізує Stage 10: персоналізацію першого дня тижня в календарі (Calendar Personalization). Користувачі можуть обрати бажаний перший день тижня — будь-який із семи днів — на сторінці налаштувань (`/settings`). Налаштування зберігається в профілі користувача через новий API-ендпоінт `PATCH /api/v1/me`. Календар тижневого розкладу автоматично відображає тиждень відповідно до персонального вибору, а глибокі посилання з параметром `?week=` автоматично прив'язуються (snapping) до початку тижня поточного глядача.

Також збережено всі можливості попередніх етапів: реєстрацію з підтвердженням email, вхід, сесії, вихід, захист маршрутів, список кімнат з фільтром за місткістю, тижневий розклад, повторювані щотижневі бронювання, сторінку `Мої бронювання` з історією та cursor-пагінацією, а також сповіщення про завершення бронювань реального часу з SSE.

Етап 11 (фіналізація та підготовка до здачі) залишається єдиним наступним етапом.
